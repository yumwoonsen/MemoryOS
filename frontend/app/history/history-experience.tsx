...
export function HistoryExperience({ initialPacks }: { initialPacks: MemoryPackV11[] }) {
  ...
  return <main className="player-app history-app" data-theme="light" data-game="free-fire">
    ...
    <div className="player-shell"><header className="player-topbar"><Link className="player-brand" href="/" aria-label="MemoryOS player home"><span className="player-brand-mark">M</span><span>MemoryOS</span></Link><div className="player-topbar-actions"><span className={`engine-badge ${view.kind === "loading" || view.kind === "sending" ? "checking" : ""}`}><i aria-hidden="true" />{view.kind === "accepted" ? "Mission ready" : "Memory inbox"}</span><Link className="player-studio-link" href="/studio" aria-label="Open the MemoryOS Developer Studio"><span className="player-studio-long">Developer </span>Studio</Link></div></header>
